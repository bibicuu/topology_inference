...
for i = 1:length(metrics)
    ...
    eval([tensor_name, ' = zeros(nr_graphs, nr_signals, nr_scenarios, length(H_plot), length(M_plot), length(M_plot));']);
						...
						if (i <= length(metrics))
							...
							for alpha_idx = 1:length(alpha_plot)
								...
								data_hid = load(sprintf('./simulations/graph%d_N%d_sc%d_%s_p%.2f_%s_weigh%d/signal%d_N%d_M%d_w%d/scenario%d_H%d_%s/sols/Cov_full_N%d_M%d_H%d_l%d_w%d_alpha%d_beta%d.mat', ...
																ID_graph, N, scale_graph, g_type, prob, symmetric, weighted, ID_signal, N, M, w, ID_scenario, H, type_of_hidden, ...
																N, M, H, lambda_scale/1e3, w, alpha, beta)).metrics;
...
for i = 1:length(metrics_name)
    ...
    mean_value = squeeze(squeeze(mean(mean(mean(mean(eval(tensor_name),4), 1), 2), 3)));
    median_value = squeeze(squeeze(median(median(median(mean(eval(tensor_name),4), 1), 2), 3)));
...
metric_figure = 'err_no_amb'; % Cambia 'err_no_amb' por cualquier métrica, como 'fscore'
alpha_idx = 1; % 1 es alpha = 0
...
% Gráfico de la media
figure;
loglog(M_plot, eval([hid_mean_name, '(:,alpha_idx)']), '-o', 'DisplayName', ['HID ', sprintf('alpha %d', alpha_plot(alpha_idx))], 'LineWidth', 1.5);
hold on;
loglog(M_plot, eval(nohid_mean_name), '-s', 'DisplayName', ['NO HID ', sprintf('alpha %d', alpha_plot(alpha_idx))], 'LineWidth', 1.5);
xlabel('M\_plot');
ylabel([metric_figure, ' mean']);
legend('show');
...
figure;
loglog(M_plot, eval([hid_median_name, '(:,alpha_idx)']), '-o', 'DisplayName',  ['HID ', sprintf('alpha %d', alpha_plot(alpha_idx))], 'LineWidth', 1.5);
hold on;
loglog(M_plot, eval(nohid_median_name), '-s', 'DisplayName', ['NO HID ', sprintf('alpha %d', alpha_plot(alpha_idx))], 'LineWidth', 1.5);
xlabel('M\_plot');
ylabel([metric_figure, ' median']);
legend('show');
